...
$count = count($info);
for ($p=0; $p < $count ; $p++) { 	
	if ($info[$p]['measure'] == 'бр.' || $info[$p]['measure'] == 'мл') {
		$info[$p]['quantity_gr'] = $info[$p]['quantity']*$info[$p]['coef_to_gr'];
	} elseif ($info[$p]['measure'] == 'гр' ) {
		$info[$p]['quantity_gr'] = $info[$p]['quantity'];
	} elseif ($info[$p]['measure']!=='бр.' && $info[$p]['measure']!=='мл' && $info[$p]['measure']!=='гр') {
...
$q_s = "SELECT SUM(`quantity_gr`) FROM `recipe_products_quantities` WHERE `recipe_id` = 11";
