...
$recipe = array(array());
$i = 0;
$q = "SELECT *, `recipes`.`date_published` as dp, `recipes`.`description` as rdesc, `users`.`username` as `ur` FROM `recipes` 
		...
		<div class="row">
			<div class="col-xs-6">
				...
				<div class='col-xs-6'>";
					...
					echo '<img width="450" src="data:image/jpeg;base64,'.base64_encode( $row_photo['content_photo'] ).'"/><br />';
					echo '<a class="btn btn-primary active" role="button" href="update_recipe_details.php?updaterd='. $id_rec .'&num='.$num.'">ПРОМЕНИ</a>';
					echo ' <a href="main.php" class="btn btn-primary active" role="button">ВСИЧКИ РЕЦЕПТИ</a>
				</div>';
				?>
				</div>
			</div>
